# Any Router Local Proxy

A small, zero-dependency Node.js HTTP proxy that sits between Claude Code and an
Anthropic-API-compatible gateway ("Any Router"). Claude Code talks to this proxy
...
| `ANYROUTER_WIRE_ARCH` | `x-stainless-arch` header value. | `arm64` |
| `ANYROUTER_STAINLESS_VERSION` | `x-stainless-package-version` header value. | `0.94.0` |
...
| `PROXY_TRACE_FILE` | Trace destination. Must be under `/tmp` in the container. | `/tmp/anyrouter-trace.log` |
...
```sh
docker compose exec proxy cat /tmp/anyrouter-trace.log
```
...
- Rotate `ANYROUTER_API_KEY` at the gateway if it has ever been committed,
...
| 401 from upstream, proxy healthy | `ANYROUTER_API_KEY` is wrong or still the placeholder. |
